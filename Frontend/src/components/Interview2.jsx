...
function Interview2({ interviewData, onFinish }) {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Fixed: Init as boolean instead of empty string
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      if (isIntroPhase) {
        ...
        await speakText(
          "I will ask you a few questions. Just answer naturally, and take your time. Let's begin.",
        );
        ...
        if (currentIndex === questions.length - 1) {
          await speakText(
            "Allright, this one might be a bit more challenging.",
          );
        }
  ...
  useEffect(() => {
    if (!isIntroPhase && currentQuestion) {
      setTimeLeft(currentQuestion.timeLimit || 60); // Fixed: mapped to timeLimit instead of non-existent timeLeft
    }
  ...
  const handleNext = async () => {
    ...
    // Fixed: Properly structured setTimeout for delayed mic activation
    setTimeout(() => {
      if (isMicOn) startMic();
    }, 500);
  };
  ...
  useEffect(() => {
    ...
    if (timeLeft === 0 && !isSubmitting && !feedback) {
      submitAnswer(); // Fixed: Changed from handleSubmit() to submitAnswer()
    }
  }, [timeLeft]);
